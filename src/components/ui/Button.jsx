...
export default function Button({ children, className = '', href, ...props }) {
  ...
  if (href === "#hero-form") {
    const telHref = `tel:${t.COMPANY.phone.replace(/\s+/g, '')}`;
    const baseWithoutFlex = combinedClasses.replace(/\bflex\b/g, '').trim();
    
    return (
      <>
        {/* Desktop: Scrolls to form */}
        <a href={href} className={`${baseWithoutFlex} hidden md:flex`} {...props}>
          {children}
        </a>
        {/* Mobile: Calls number */}
        <a href={telHref} className={`${baseWithoutFlex} flex md:hidden`} {...props}>
          {children}
        </a>
      </>
    );
